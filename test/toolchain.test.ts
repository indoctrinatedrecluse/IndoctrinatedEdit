...
describe('Compiler & SDK Auto-Detection Framework', () => {
  ...
  it('should safely execute all extension toolchain triggers without TDZ errors', async () => {
    const { triggerGoToolchainCheck } = await import('../src/extensions/goSupport/goExtension')
    const { triggerRustToolchainCheck } = await import('../src/extensions/rustSupport/rustExtension')
    const { triggerCppToolchainCheck } = await import('../src/extensions/cppSupport/cppExtension')
    const { triggerPythonToolchainCheck } = await import('../src/extensions/pythonSupport/pythonExtension')
    const { triggerJavaToolchainCheck } = await import('../src/extensions/javaSupport/javaExtension')
    const { triggerDotnetToolchainCheck } = await import('../src/extensions/dotnetSupport/dotnetExtension')
    const { triggerPhpToolchainCheck } = await import('../src/extensions/phpSupport/phpExtension')
    const { triggerAngularToolchainCheck } = await import('../src/extensions/angularSupport/angularExtension')
    const { triggerReactToolchainCheck } = await import('../src/extensions/reactSupport/reactExtension')
    const { triggerFlutterToolchainCheck } = await import('../src/extensions/flutterSupport/flutterExtension')
    const { triggerRubyToolchainCheck } = await import('../src/extensions/rubySupport/rubyExtension')
    const { triggerNodeToolchainCheck } = await import('../src/extensions/nodeSupport/nodeExtension')
