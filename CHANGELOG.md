...
- **Pre-Optimized xterm.js + ConPTY Engine**:
  - Integrated Microsoft's industry-standard `@xterm/xterm` canvas terminal engine paired with `@xterm/addon-fit` and `node-pty` (Windows ConPTY / POSIX openpty).
  - True `isatty = true` pseudoconsole execution across all shell environments (PowerShell 7, Windows PowerShell, Git Bash, Cygwin, WSL, CMD, Linux/macOS shells) for instant, real-time unbuffered interactive output, live prompts (`PS C:\>`), and full keyboard routing.
  - Native support for interactive full-screen CLI/TUI applications (`ir pmon`, `ir nettop`, `ir dua`, `ir fm`, `ir edit`, `ir matrix`, `ir help`, `nano`, `htop`, `vim`, etc.) with 100% ANSI/VT100 compatibility and alternate screen buffers.
...
#### 🌌 Google Antigravity Studio, Gemini 3.7 Flash & OAuth2 Integration
- **Flagship Gemini 3.7 Flash Model**: Powered by Google's latest `gemini-3.7-flash` model with hybrid Chain-of-Thought (CoT) reasoning tokens, 1M-token context window, and ultra-fast streaming.
- **Stateful Sliding-Window Context Compaction**: Built-in intelligent conversation context compaction preserving recent turns in full fidelity while compacting older history (>16k tokens) into anchor summaries to prevent token bloat without losing conversational continuity.
- **Full Interactive Chat Studio**: Multi-turn pair programming interface with syntax-highlighted code blocks, one-click "Copy Code" & "Insert into Editor", expandable Chain-of-Thought reasoning inspector, quick prompt chips, model selector, real-time token estimation, and active file context attachment.
- **Google AI Studio Key / Antigravity Token Fallback**: Added a dedicated API Key management section directly in the Settings Drawer to support instant zero-scope-limitation personal account access alongside browser loopback OAuth2.
- **Resilient Error Stream Handling**: Gracefully catches authentication and scope errors, rendering actionable diagnostic guidance and fallback steps directly inside chat bubbles rather than leaving silent hanging bubbles.
- **Authentic Google OAuth2 Loopback Authentication**: Authenticate directly with personal Google accounts via local browser loopback flow with multi-scope support (`generative-language`, `cloud-platform`, `userinfo.profile`, `userinfo.email`), retrieving authentic Google user profiles with seamless fallback to Application Default Credentials (ADC) and `gcloud` active projects.
- **Title Bar & Viewport Layout Containment**: Fixed window viewport scrolling when rendering or streaming chats by scoping all auto-scroll operations strictly to the inner message list viewport, preventing displacement of the frameless title bar.
- **Live Python Daemon Logs & Diagnostics Viewer**: Real-time structured log streaming and inspection dialog (`GET /debug/logs` & `antigravity:getLogs`) with copy to clipboard, auto-refresh polling, and colorized log level markers (`[INFO]`, `[ERROR]`, `[DEBUG]`, `[SSE]`, `[AUTH]`) written to `~/.indoctrinated/antigravity_backend.log`.
- **Zero Boot Overhead & Resilient Discovery**: Backend service files and the Python SDK harness are **strictly loaded on-demand** into working memory only when needed. Added Windows Python runtime discovery across `AppData\Local\Programs\Python`, `Program Files`, and system PATH, and bundled `sidecars/` via `extraResources` in electron-builder.
- **Python Antigravity SDK Sidecar Daemon**: High-performance local sidecar daemon interfacing `google-antigravity` and `google-genai` with streaming SSE responses (`/v1/chat/stream`), tokenization analytics (`/v1/tokenize`), quota telemetry (60 RPM / 1M token context tracking), and real-time model metrics.
