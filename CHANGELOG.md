...
- **Full Interactive Chat Studio**: Multi-turn pair programming interface with syntax-highlighted code blocks, one-click "Copy Code" & "Insert into Editor", expandable Chain-of-Thought (CoT) reasoning inspector, quick prompt chips, model selector, real-time token estimation against the 1M token limit, and active file context attachment.
